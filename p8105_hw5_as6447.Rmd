...
```
We create the `total_number_homicides` variable through the `summarize` function. The `number_unsolved_murders` variable was created through the `summarization` function. 
```{r}
homicide_data1=
homicide_data%>%
   group_by(city_state)%>%
...
2.For each dataset, save μ^ and the p-value arising from a test of H:μ=0 using α=0.05. 
